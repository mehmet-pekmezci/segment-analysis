...
if __name__ == "__main__":
    ...
    for iter in range(1):
        ...
        dir_arr = [name for name in os.listdir("../output") if name.startswith("five-segment") ]
        #dir_arr = [name for name in os.listdir("/Users/mehmetpekmezci/ozu/segment-analysis") if name.startswith("five-segment") ]
        for dir_name in dir_arr:
            ...
            third_segment_size = int(dir_name.split("-")[4])
            fourth_segment_size = int(dir_name.split("-")[5])
            fifth_segment_size = int(dir_name.split("-")[6])
            node_count = int(dir_name.split("-")[7])

            second_segment_start = first_segment_size
            third_segment_start = second_segment_start + second_segment_size
            fourth_segment_start = third_segment_start + third_segment_size
            fifth_segment_start = fourth_segment_start + fourth_segment_size

            segment_borders = [[0,second_segment_start],
            ...
            obs_idx_arr = [int(first_segment_size/2),
                           ...
                           int(fourth_segment_size/2) + fourth_segment_start,
                           int(fifth_segment_size/2) + fifth_segment_start
                           ]
            for idx in range(1):
                cumulative_offset = 0
